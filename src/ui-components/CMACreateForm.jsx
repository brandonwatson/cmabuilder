...
export default function CMACreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
  ...
  const resetStateValues = () => {
    ...
    setCma_label(initialValues.cma_label);
    setClient_name(initialValues.client_name);
    setErrors({});
  };
  const validations = {
    pk: [{ type: "Required" }],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(new CMA(modelFields));
          if (onSuccess) {
      ...
      {...getOverrideProps(overrides, "CMACreateForm")}
      {...rest}
    >
      <TextField
        label="Pk"
